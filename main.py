...
# Frontend is served from this same app in production, so CORS is mostly a
# safety net for local dev (opening index.html directly / a separate dev
# server on a different port). Override via the ALLOWED_ORIGINS env var
# (comma-separated) if you ever split the frontend onto its own domain.
...
# /api/* route above -- this lets one deployed service serve both the API
# and the frontend from a single origin (no CORS, one URL to host).
_static_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "static")
if os.path.isdir(_static_dir):
    app.mount("/", StaticFiles(directory=_static_dir, html=True), name="static")
else:
    logger.warning("static/ directory not found at %s; dashboard will not be served.", _static_dir)
